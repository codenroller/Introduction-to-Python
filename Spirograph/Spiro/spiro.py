...
class SpiroAnimator:
    ...
    def __init__(self, N):
        ...
        # call timer
        self.s.ontimer(self.update, self.deltaT)
...
def saveDrawing():
    ...
    # hiding active turtle
    active_turtles = []
    for t in s.turtles():
        if t.isvisible():
            active_turtles.append(t) 
            t.ht()
    ...
    # show active turtles that were hidden
...
def main():
    ...
    if args.sparams:
        ...
        s.onkey(spiroAnim.toggleTurtles, "t")
        # add key handler to restart animation
        s.onkey(spiroAnim.restart, "space")
